...
pub struct UserDb { connection: Mutex<Connection> }
...
impl UserDb {
    ...
    pub fn insert_bookmark(&self, source_id: &str, ref_name: &str, document_path: &str) -> Result<(), XenicsError> {
    ...
    pub fn bookmark_count(&self) -> Result<i64, XenicsError> {
        let connection = self.connection.lock().expect("user database mutex");
        connection.query_row("SELECT COUNT(*) FROM bookmarks", [], |row| row.get(0)).map_err(database_error)
    }
