...
The reader accepts only the parsed native document model. React renders text nodes and structured blocks, maps parser warnings to visible warning blocks, resolves internal links into the active tab history, and exposes per-tab back/forward navigation with stale forward-history truncation. Reader zoom is bounded to 80–140% and passed as presentation state; code blocks render line numbers and a user-controlled wrapping mode. The renderer never evaluates repository HTML or MDX as executable markup.
