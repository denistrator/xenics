...
export function TaskPanel({ tasks, onCancel, onRetry }: TaskPanelProps) {
  return (
    <section aria-labelledby="tasks-title" className="space-y-4">
      ...
      {tasks.length === 0 ? <p className="rounded-xl border border-x-line bg-x-panel p-6 text-sm text-x-muted">No recent tasks.</p> : <div className="space-y-3">{tasks.map((task) => <TaskRow key={task.taskId} task={task} onCancel={onCancel} onRetry={onRetry} />)}</div>}
    </section>
  )
}
